...
export default class StringsProvider {
    ...
    private _fallBackMap: Map<string, Array<string>> = new Map<string, Array<string>>();
    ...
    public static getFallback(locale: string | undefined): string {
        if (locale && StringsProvider.instance) {
            let fallbackLocale: string[] | undefined = StringsProvider.instance._fallBackMap.get(locale);
